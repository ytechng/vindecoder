...
public class DecoderDAOImpl implements DecoderDAO {
	...
	// String selectQuery = "SELECT Decoder WHERE vin = :vin and status =
	// :status";
	// try {
	// return sessionFactory.getCurrentSession().get(Decoder.class,
	// Integer.valueOf(vin));
	//
	// } catch(Exception e) {
	// e.printStackTrace();
	...
	public Decoder get(String vin) {
		...
		String query = "FROM Decoder WHERE vin = :vin";

		try {

			return sessionFactory.getCurrentSession()
					.createQuery(query, Decoder.class)
